...
# python preprocess.py -train_src train_inputs.txt -train_tgt train_labels.txt -valid_src valid_inputs.txt -valid_tgt valid_labels.txt -test_src test_inputs.txt -test_tgt test_labels.txt -save_data train_valid_test.pt
    ...
    with open(inst_file) as f:
        for sent in f:
            ...
            if word_inst:
                ...
                word_insts += [[Constants.BOS_WORD] + [Constants.EOS_WORD]]
...
def read_tabular_instances_from_file(inst_file, max_sent_len, keep_case, use_bos_eos=True):
    ''' Convert file into word seq lists and vocab '''
    ...
    with open(inst_file) as f:
        for sent in f:
            words = sent.split()
            # stop()
            word_inst = [float(i) for i in words]
            # word_inst = words
...
def main():
    ...
    if opt.train_adj:
        train_src_word_insts = read_tabular_instances_from_file(opt.train_src, opt.max_seq_len, opt.keep_case,use_bos_eos=False)
        train_adj_insts = read_adj_matrix(opt.train_adj)
    else:
        train_src_word_insts = read_tabular_instances_from_file(opt.train_src, opt.max_seq_len, opt.keep_case)
    ...
    if opt.valid_adj:
        valid_src_word_insts = read_tabular_instances_from_file(opt.valid_src, opt.max_seq_len, opt.keep_case,use_bos_eos=False)
        valid_adj_insts = read_adj_matrix(opt.valid_adj)
    else:
        valid_src_word_insts = read_tabular_instances_from_file(opt.valid_src, opt.max_seq_len, opt.keep_case)
    ...
    train_src_insts = train_src_word_insts
    valid_src_insts = valid_src_word_insts
    ...
    if opt.test_adj:
        ...
        test_src_word_insts=read_tabular_instances_from_file(opt.test_src,opt.max_seq_len,opt.keep_case,use_bos_eos=False)
    else:
        test_src_word_insts=read_tabular_instances_from_file(opt.test_src,opt.max_seq_len,opt.keep_case)

    
    test_src_insts=test_src_word_insts
