...
# python preprocess.py -train_src train_inputs.txt -train_tgt train_labels.txt -valid_src test_inputs.txt -valid_tgt test_labels.txt -test_src test_inputs.txt -test_tgt test_labels.txt -save_data train_valid_test.pt
...
def read_tabular_instances_from_file(inst_file, max_sent_len, keep_case, use_bos_eos=True):
    ...
    with open(inst_file) as f:
        for sent in f:
            words = sent.split()
            # stop()
            word_inst = [str(i) for i in words]
            # word_inst = words
